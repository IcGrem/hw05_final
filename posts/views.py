...
def post_edit(request, username, post_id):
    ...
    if request.user != profile:
        return redirect('post', username=profile, post_id=post_id)
...
def add_comment(request, username, post_id): #username это post.author.username - автор поста
    ...
    return render(request, 'post.html', {'username': username, 'profile': profile,
...
def follow_index(request):
    ...
    return render(request, 'follow.html', {'page': page, 'paginator': paginator,
                                           'post': following_list, 'username': user.username})
